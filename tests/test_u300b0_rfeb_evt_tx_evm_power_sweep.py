...
def test_u300b0_rfeb_evt_tx_evm_power_sweep(cmw100, wfg, dut, dmm_c, dmm_v, config, results):
    ...
    # Step 4 of bench-validation cleanup — was: Setup_DMM().
    # Only dmm_c is wired in V1b; engineer adds dmm_idd1v4/1v8/2v5/iapt/ibat/ifem*
    # to the dmms dict when those fixtures land in V2.
    setup_dmm(dmms={"dmm_c": dmm_c})

    # Config field renames (V1c migrator stripped 'in_' but field names differ in OpenFlowConfig):
    tc_cond_limits = Testconditions_Limits(config.limits_path)
    target_tx_power = config.tx_power_dBm

    # read deembedding (loss) data
    deembedding = Deembedding(config.deembedding_path)
    ...
    cal_file = Calibration_File(config.calibration_path, config.rfeb_sn, config.rfhb_sn)
    ...
    for modulation in ["16QAM"]:
        ...
        target_evm_max = tc_cond_limits.get_band_modulation(CLASS_NAME, band=config.band,
                                        modulation=modulation, param = 'EVM_MAX')
        target_evm_margin = tc_cond_limits.get_band_modulation(CLASS_NAME, band=config.band,
        ...
        for target_tx_power in np.arange(-45, 28+1, 1.0):
            ...
            aux_readings = get_aux(dut)
            dmm_readings = get_dmm(dmms={"dmm_c": dmm_c})

            # Set verdict — dut.get_rfTxMPR / get_rfTxPowerMax not in V1b DUT_U300 port;
            # they raise NotImplementedError. Fall back to safe defaults so the EVM
            # comparison still runs. Engineer should replace with real methods once ported.
            try:
                ...
                logger.warning("get_rfTxMPR/get_rfTxPowerMax not ported: %s — using safe defaults", e)
                txmpr = 0.0
                txpowermax = 28.0  # top of the sweep range
